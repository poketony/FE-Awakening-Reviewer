...
export function buildSafeEditTemplate(source) {
  ...
  for (let index = 0; index < script.length;) {
    if (script.startsWith("\\n", index)) {
      // Show a game line break as a normal textarea line break so a two-line
      // message can be edited in one field. Rebuild restores the locked token.
      if (buffer.length) {
        buffer += "\n";
      } else {
        const token = "\\n";
...
export function rebuildSafeEditTemplate(template, values) {
  ...
  return template.parts.map((part) => {
    ...
    const value = String(replacements[part.textIndex] ?? part.value);
...
export function validateSafeText(value) {
  ...
  const errors = [];
  if (text.includes("\r")) errors.push("지원하지 않는 줄바꿈 문자가 포함되어 있습니다.");
