...
export class GitHubClient {
  ...
  async getTree(ref = "main") {
    ...
    try {
      ...
      const stateProgress = stateTree.find((entry) => entry.path === REVIEW_PROGRESS_PATH && entry.type === "blob");
      if (!stateProgress) return tree;
      return [...tree.filter((entry) => entry.path !== REVIEW_PROGRESS_PATH), stateProgress];
    } catch {
      return tree;
    }
  }
  ...
  async commitReviewProgress(file, message) {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        ...
        const currentTree = await this.getTreeRaw(head.commitSha);
        const descriptor = currentTree.find((entry) => entry.path === REVIEW_PROGRESS_PATH && entry.type === "blob");
        const remoteText = descriptor ? await this.getBlobText(descriptor.sha) : `${JSON.stringify(emptyProgress(), null, 2)}\n`;
        ...
        const tree = await this.request(this.repoPath("/git/trees"), {
          ...
          body: JSON.stringify({
            ...
            tree: [{ path: REVIEW_PROGRESS_PATH, mode: "100644", type: "blob", sha: blob.sha }],
          }),
        });
        const commit = await this.request(this.repoPath("/git/commits"), {
          method: "POST",
    ...
    throw lastError || new Error("검수 기록 동기화에 실패했습니다.");
  }
  ...
  async commitFilesToBranch({ files, branch = "main", message }) {
    ...
    if (reviewFiles.length) reviewResult = await this.commitReviewProgress(reviewFiles.at(-1), `검수 기록 동기화 · ${message}`);
