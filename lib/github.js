...
const VIRTUAL_REVIEW_SHA = "__fe_review_progress_union__";
...
export class GitHubClient {
  ...
  async buildReviewUnion(stateTree = null) {
    ...
    const read = async (path) => {
      ...
      return descriptor?.type === "blob" ? this.getBlobTextRaw(descriptor.sha) : `${JSON.stringify(emptyProgress(), null, 2)}\n`;
    };
    const [legacy, pc, mobile] = await Promise.all([
      read(REVIEW_LEGACY_PATH),
      read(REVIEW_PC_PATH),
      read(REVIEW_MOBILE_PATH),
    ]);
  ...
  async getTree(ref = "main") {
    ...
    try {
      ...
      return [
        ...
        { path: REVIEW_PROGRESS_PATH, mode: "100644", type: "blob", sha: VIRTUAL_REVIEW_SHA },
      ];
    } catch {
      return tree;
    }
  ...
  async getBlobText(sha) {
    if (sha === VIRTUAL_REVIEW_SHA) {
      if (!this.virtualReviewText) await this.buildReviewUnion();
      return this.virtualReviewText;
    }
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        ...
        if (mergeProgressText(remoteText, remoteText) === mergedText) {
          this.virtualReviewText = file.text;
          return {
