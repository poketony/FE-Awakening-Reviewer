...
export class GitHubClient {
  ...
  async commitFilesToBranch({ files, branch = "main", message }) {
    ...
    // HEAD와 트리를 같은 스냅샷에서 읽고, 번역 초안은 검수 시작 당시 SHA와 비교한다.
    const head = await this.getHead(branch);
    const currentTree = await this.getTree(head.commitSha);
    ...
    // force=false이므로 그 사이 main이 움직였으면 fast-forward가 아니어서 GitHub가 거부한다.
  ...
  // 예전 PR 흐름과의 호환용. 새 모바일 UI에서는 사용하지 않는다.
