...
function isAbortLike(error) {
  ...
  return name === "AbortError"
    || /\babort(?:ed|ing)?\b/iu.test(message)
    || message.includes("__review_switch_aborted__");
}
...
export class GitHubClient {
  ...
  async request(path, options = {}) {
    ...
    const critical = this.criticalNetworkDepth > 0;
    const nativeFetch = typeof globalThis.__reviewerNativeFetch === "function"
      ? globalThis.__reviewerNativeFetch
      : null;
    const requestFetch = critical && nativeFetch ? nativeFetch : fetch;
    const requestOptions = { ...options, headers };

    let response = null;
    for (let attempt = 0; attempt < (critical ? 3 : 1); attempt += 1) {
      try {
        response = await requestFetch(url, requestOptions);
        break;
      } catch (error) {
        if (!critical || !isAbortLike(error) || attempt >= 2) throw error;
        await delay(140 * (attempt + 1));
      }
    }
    if (!response) throw new Error("GitHub 요청 응답을 받지 못했습니다.");
    if (!response.ok) {
      let detail = "";
  ...
  async withCriticalNetwork(callback) {
    this.criticalNetworkDepth += 1;
    try {
      return await callback();
    } finally {
      this.criticalNetworkDepth = Math.max(0, this.criticalNetworkDepth - 1);
    }
  }
  ...
  async commitFilesToBranch({ files, branch = "main", message }) {
    ...
    return this.withCriticalNetwork(async () => {
