...
function isCanonicalProgress(value) {
  return Boolean(
...
function progressView(value) {
  return isCanonicalProgress(value) ? value : parseProgress(value);
}
...
export function mergeProgress(base, incoming) {
  const left = progressView(base);
  const right = progressView(incoming);
  const output = emptyProgress();
...
export function registerFile(progress, { path, mode = "main", expected = [], at = new Date().toISOString() }) {
  if (!path || !progress || typeof progress !== "object") return false;
  if (!isCanonicalProgress(progress)) Object.assign(progress, parseProgress(progress));
  const id = makeFileId(path);
  const nextExpected = uniqueStrings(expected);
  const current = progress.files[id];
  const normalizedMode = mode === "dlc" ? "dlc" : "main";
  const same = current
    && current.path === path
...
export function setReviewStatus(progress, {
  ...
  if (!path || !entryKey || !STATUS_SET.has(status) || !progress || typeof progress !== "object") return false;
  if (!isCanonicalProgress(progress)) Object.assign(progress, parseProgress(progress));
...
export function getReviewStatus(progress, path, entryKey) {
  const parsed = progressView(progress);
  return parsed.entries[makeEntryId(path, entryKey)]?.status || "unreviewed";
}

function fileProgressFromParsed(parsed, path) {
...
export function fileProgress(progress, path) {
  return fileProgressFromParsed(progressView(progress), path);
}
...
export function summarizeMode(progress, mode, catalogPaths = []) {
  ...
  for (const path of paths) {
    const summary = fileProgressFromParsed(parsed, path);
